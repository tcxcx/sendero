...
import type { BridgeResult, BridgeStep, SwapResult } from '@circle-fin/app-kit';
import { createViemAdapterFromPrivateKey, type ViemAdapter } from '@circle-fin/adapter-viem-v2';
import { ArcTestnet } from '@circle-fin/app-kit/chains';
import { createPublicClient, http, type Chain as ViemChain, type PublicClient } from 'viem';
...
export function getTreasuryAddress(): string {
  ...
  if (!a)
    throw new Error('TREASURY_VIEM_ADDRESS (or CIRCLE_TREASURY_ADDRESS) not set in .env.local.');
  return a;
}

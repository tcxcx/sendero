...
export const idDocumentSchema = z.object({
  ...
  date_of_birth: z.string().nullable().describe('ISO 8601 date of birth (YYYY-MM-DD)'),
  sex: z.enum(['M', 'F', 'X']).nullable().describe('Sex as printed on document'),
  nationality: z.string().nullable().describe('ISO 3166-1 alpha-3 nationality code'),
  ...
  mrz_line1: z.string().nullable().describe('First Machine Readable Zone line, verbatim'),

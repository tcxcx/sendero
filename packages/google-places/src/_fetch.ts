/**
 * @sendero/google-places/_fetch — Bun-fetch-vs-curl-fallback HTTP helper.
 *
 * Mirrors `@sendero/web-search/_fetch` exactly (which itself mirrors
 * `@sendero/arize-phoenix/_fetch`). Bun's HTTP/2 fetch fingerprint can
 * trip aggressive Cloudflare WAFs even with valid auth — curl over
 * HTTP/1.1 sails through. We try native first, fall back on detected
 * bot-challenge (200 + text/html when JSON requested).
 *
 * places.googleapis.com is generally clean; this fallback exists for
 * symmetry + future-proofing. `via` field on the response surfaces
 * which path served the request so we can confirm production stays
 * native.
 */
  ...
  return new Promise<SenderoFetchResponse>(resolve => {
    ...
    proc.on('close', exitCode => {
      if (exitCode !== 0 && !stdout) {
        resolve(makeResponse(false, 0, 'error', `curl exit ${exitCode}: ${stderr.slice(0, 400)}`));
        return;
      }

 ...
 *
 * Phase 2 started ARC-only by configuration; the abstraction is what
 * Phase 3 + Phase 4 widen as additional Gateway chains are enabled.
 */

import { prisma } from '@sendero/database';
import { getTenantOperationsChains, getTenantTreasuryChains } from '@sendero/env/chains';
    ...
    try {
      ...
      if (msg.includes('P2002') || msg.includes('Unique constraint')) {
        const winner = await prisma.circleWallet.findFirst({
          where: { tenantId: args.tenantId, kind: 'operations', chain },
          select: { id: true },
        });
        if (winner) continue;
      }

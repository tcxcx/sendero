...
import { parseCredentialPublicKey, serializePublicKey } from 'webauthn-p256';
...
export function isPasskeyConfigured(): boolean {
  ...
  return clientKey.startsWith('TEST_CLIENT_KEY:') || clientKey.startsWith('LIVE_CLIENT_KEY:');
}
...
export function passkeyConfigIssue(): string | null {
  ...
  if (!clientKey.startsWith('TEST_CLIENT_KEY:') && !clientKey.startsWith('LIVE_CLIENT_KEY:')) {
...
async function createLocalPasskey(displayName: string): Promise<StoredCredential> {
  ...
  const publicKey = serializePublicKey(await parseCredentialPublicKey(publicKeyBuf), {
    compressed: false,
  }) as Hex;
...
async function assertLocalPasskey(credentialId?: string, rpId?: string): Promise<string> {
...
export async function registerPasskey(profile: UserProfile): Promise<UserWallet> {
  ensureConfigured();
...
export async function loginPasskey(): Promise<UserWallet> {
  ...
  if (!stored) {
    throw new Error('No passkey on this device. Use "Create a passkey" first.');
  }
...
function loadStoredCredential(): StoredCredential | null {
  ...
  try {
    ...
    if (parsed && typeof parsed.id === 'string' && typeof parsed.publicKey === 'string') {
      return {
        id: parsed.id,

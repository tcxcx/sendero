...
export function encodeApproveUsdc(spender: Address, amount: bigint): EncodedCall {
  return {
    to: ARC_USDC_ADDRESS,
 ...
 */
export function encodeUsdcTransfer(recipient: Address, amount: bigint): EncodedCall {
  return {
    to: ARC_USDC_ADDRESS,
    data: encodeFunctionData({
      abi: USDC_ABI,
...
export function encodeComplete(jobId: bigint, reasonHash: Hex): EncodedCall {
  return {
    to: AGENTIC_COMMERCE_ADDRESS,
    data: encodeFunctionData({
      abi: AGENTIC_COMMERCE_ABI,

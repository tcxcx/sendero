      ...
      <div className="itin-grid">
        {offers.map((offer) => {
          const dep = offer.departure
            ? new Date(offer.departure).toISOString().slice(11, 16)
            : '—';
          const arr = offer.arrival
            ? new Date(offer.arrival).toISOString().slice(11, 16)
            : '—';
          ...
          return (
            <div
              key={offer.id}
              className={`itin-leg ${isSelected ? 'selected' : ''}`}
            >
              <div className="itin-times">
                ...
                <div className="itin-tcol">
                  ...
                  <div className="itin-iata">
                    {offer.destinationCode ?? 'ARR'}
                  </div>
                </div>
              </div>
...
function HoldCard({
  ...
  const deadline = holdOrder.paymentRequiredBy
    ? new Date(holdOrder.paymentRequiredBy)
    : null;
    ...
    <div className="card">
      ...
      <div className="settle-grid">
        ...
        <div className="settle-cell">
          ...
          <span className="v">
            {deadline
              ? deadline.toISOString().slice(11, 16) + ' UTC'
              : '—'}
          </span>
        ...
        <div className="settle-cell">
          ...
          <span className="k">
            {paid ? 'seat confirmed' : 'awaiting balance'}
          </span>
        </div>
      </div>
      {!paid && (
        ...
        <div className="settle-grid">
          ...
          <div className="settle-cell">
            ...
            <span className="v mono-v">
              {onChain?.deliverableHash
                ? `${onChain.deliverableHash.slice(0, 10)}…`
                : '—'}
            </span>
...
function HotelsCard() {
  ...
  const nights = search
    ? Math.max(
        1,
        Math.round(
          (new Date(search.checkOutDate).getTime() -
            new Date(search.checkInDate).getTime()) /
            (1000 * 60 * 60 * 24),
        ),
      )
    : null;

...
export function BridgeDialog() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          data?.message || data?.error || `bridge_failed (${res.status})`,
        );
      }
  ...
  return (
    ...
    >
      <p className="dlg-sub">
        Pulls USDC from another supported chain into Arc Testnet via Circle
        CCTP. Treasury wallet is both source and destination for the demo.
      </p>

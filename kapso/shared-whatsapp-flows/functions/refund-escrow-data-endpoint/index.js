...
async function handler(request, env) {
  ...
  const screen = exchange.screen || 'REFUND_ESCROW_TYPE';
  ...
  if (action === 'INIT') {
    return json({ version: '3.0', screen: 'REFUND_ESCROW_TYPE', data: { error_message: '' } });
  }
  if (action === 'BACK') return json({ version: '3.0', screen, data });

  if (screen === 'REFUND_ESCROW_TYPE') {
    if (!text(data.trip_id) || !text(data.request_type) || !text(data.urgency)) {
      return json({
        version: '3.0',
        screen,
        data: { error_message: 'Trip or escrow ID, request type, and urgency are required.' },
      });
    }
    return json({
      version: '3.0',
  ...
  if (screen === 'REFUND_ESCROW_DETAILS') {
    if (!text(data.details)) {
      return json({
        version: '3.0',
        screen,
        data: { ...data, error_message: 'Details are required.' },
      });
    }
    const persistence = await persist(
      {
        trip_id: text(data.trip_id),
        title: `Refund or escrow: ${text(data.request_type)}`,
        summary: [
          `Trip/escrow ID: ${text(data.trip_id)}`,
          `Request: ${text(data.request_type)}`,
          `Urgency: ${text(data.urgency)}`,
          text(data.reference) ? `Reference: ${text(data.reference)}` : null,
          `Details: ${text(data.details)}`,
          'Note: actual refunds, releases, settlement, and policy overrides require human review and secure approval.',
        ]
          .filter(Boolean)
          .join('\n'),
...
globalThis.__senderoRefundEscrowFlowEndpoint = { handler };

...
import { getMeterEvents, meterSummary, subscribeMeter } from '@sendero/tools/meter';
...
export function mountPaidTools(app: Hono): void {
  ...
  app.get('/tools', c => {
    return c.json({
      seller: process.env.SENDERO_SELLER_ADDRESS ?? process.env.TREASURY_VIEM_ADDRESS ?? null,

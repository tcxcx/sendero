...
import { z } from 'zod';
import { invalidateReputationCache } from '@sendero/arc/identity';

const BodySchema = z.object({
  agentId: z.string().regex(/^\d+$/, 'agentId must be a decimal string').optional(),
});
...
export async function POST(req: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          message: 'Provide agentId in the request body or set SENDERO_AGENT_ID in .env.local.',
        },
        { status: 503 }
      );
    }
    ...
    return NextResponse.json({ error: 'finalize_failed', message }, { status: 500 });
  }
}

...
import { z } from 'zod';
import { setBudget, toUsdcUnits } from '@sendero/arc/jobs';

const BodySchema = z.object({
  jobId: z.string().regex(/^\d+$/, 'jobId must be a decimal string'),
  totalAmountUsdc: z.string().regex(/^\d+(\.\d+)?$/, 'totalAmountUsdc must be a decimal string'),
});
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'set_budget_failed', message }, { status: 500 });
  }
}

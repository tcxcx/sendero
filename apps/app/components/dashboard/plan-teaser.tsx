 ...
 *
 * Click surfaces open Clerk's OrganizationProfile as an overlay dialog
 * (`useClerk().openOrganizationProfile()`). That modal carries the
 * built-in Billing tab, so subscription management lives in one place
 * and the dedicated `/dashboard/billing/plans` page is retired.
 *
 * Kept client-only because `openOrganizationProfile` is a Clerk-JS
 * method; the parent server page passes the resolved tier in as a prop.
 *
 * Hover choreography (Emil-style): tier cards use `group/plan` + a
 * small arrow translate and letter-spacing easing on "Upgrade →" so
 * each card reads as a distinct hover-target.
 */

...
export const useSendero = create<SenderoState>(set => ({
  ...
  setSettlementPhase: phase => set(s => ({ settlement: { ...s.settlement, phase } })),
  setSettlementJobId: jobId => set(s => ({ settlement: { ...s.settlement, jobId } })),
  ...
  setOffers: offers => set({ offers, status: offers.length > 0 ? 'selected' : 'idle' }),
  ...
  logEvent: e =>
    set(state => ({
      workflow: [...state.workflow, { ...e, id: `evt_${++eventCounter}` }],
    })),

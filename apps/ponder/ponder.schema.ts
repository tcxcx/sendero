...
// ════════════════════════════════════════════════════════════════════
// v3.0.0 — claim-code lockout audit trail
//
// `claimAttempt` is one row per `ClaimAttemptFailed` (rolling counter
// for analytics — the contract resets the on-chain count on success).
// `claimLockout` is one row per `ClaimLockoutTriggered`, keyed on
// `(tripId, lockedUntil)` so re-processing the same log is a no-op.
// `claimCodeRotation` is one row per `ClaimCodeRotated` for cache
// invalidation downstream.
// ════════════════════════════════════════════════════════════════════

export const claimAttempt = onchainTable(
  'claim_attempt',
  t => ({
    id: t.text().primaryKey(), // `${txHash}-${logIndex}`
    tripId: t.hex().notNull(),
...
export const claimLockout = onchainTable(
  'claim_lockout',
  t => ({
    /** Composite dedup key: `${tripId}-${lockedUntil}`. */
    ...
    blockNumber: t.bigint().notNull(),
    timestamp: t.bigint().notNull(),
    /** Status of the off-chain notification dispatch. */
    dispatchStatus: t.text().notNull(), // 'pending'|'dispatched'|'failed'|'skipped'
    dispatchError: t.text(),
    dispatchedAt: t.bigint(),
  }),
  t => ({
    byTrip: index().on(t.tripId),
    byStatus: index().on(t.dispatchStatus),
  })
);

export const claimCodeRotation = onchainTable(
  'claim_code_rotation',
  t => ({
    id: t.text().primaryKey(), // `${txHash}-${logIndex}`
    tripId: t.hex().notNull(),
    oldCodeHash: t.hex().notNull(),
    newCodeHash: t.hex().notNull(),
    txHash: t.hex().notNull(),
    blockNumber: t.bigint().notNull(),
    timestamp: t.bigint().notNull(),
  }),

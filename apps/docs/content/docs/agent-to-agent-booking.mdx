...
| Workflow delegation | Production agents that want Sendero to run the whole booking lifecycle with pauses, webhooks, and invoices. | `POST https://www.sendero.travel/api/workflows/run` |
...
5. Sendero searches Duffel inventory with `search_flights`.
6. Sendero checks policy with `check_policy`.
7. Sendero reserves upper-bound escrow with `reserve_booking`.
8. Sendero holds the Duffel offer with `book_flight`.
9. Sendero commits the actual vendor amount with `commit_booking`.
10. The workflow pauses until the Duffel ticketing webhook arrives.
11. If ticketed, Sendero calls `confirm_duffel`, `settle_booking`, and `generate_booking_invoice`.
12. If ticketing fails, Sendero calls `cancel_booking` and refunds unspent escrow.

The same workflow can pause for Slack approvals, traveler OTP, Duffel ticketing, or other external events. Do not reimplement those waits in your own agent unless you need custom orchestration.
...
```bash
curl https://www.sendero.travel/api/workflows/list
```
...
```bash
curl -X POST https://www.sendero.travel/api/workflows/run \
  -H "content-type: application/json" \
...
```bash
curl -X POST https://www.sendero.travel/api/workflows/resume \
  -H "content-type: application/json" \
  -H "authorization: Bearer $SENDERO_INTERNAL_TOKEN" \
  -d '{
    "sessionId": "ses_...",
...
Duffel ticketing usually resumes through Sendero's Duffel webhook automatically. Slack approval buttons and WhatsApp claim links also resume the same workflow runner.

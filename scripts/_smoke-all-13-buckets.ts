...
} from '../packages/tools/src/anticipation/b9-logistics';
// B10
import {
  supplierQuoteComparatorTool,
  agencyMarginGuardTool,
} from '../packages/tools/src/anticipation/b10-agency-ops';
...
function check(bucket: string, tool: string, ok: boolean, detail?: string) {
  ...
  console.log(
    `${ok ? '✓' : '✗'} [${bucket.padEnd(3)}] ${tool.padEnd(35)}${detail ? `  ${detail}` : ''}`
  );
}
...
check(
  ...
  hp1.status === 'ok',
  hp1.status === 'ok' ? `${hp1.newPreferences.length} new prefs` : ''
);
...
check(
  ...
  hp2.status === 'ok',
  hp2.status === 'ok'
    ? `tier=${hp2.budgetTier} $${hp2.expectedSpendPerPerson?.low}-${hp2.expectedSpendPerPerson?.high}`
    : ''
);

// HP3 — date_budget_optimizer
const hp3 = await runDateBudgetOptimizer({ vibe: 'romantic', budgetTier: 'medium' } as never, ctx);
check(
  'HP3',
  'date_budget_optimizer',
  hp3.status === 'ok',
  hp3.status === 'ok' ? `${hp3.moves?.length ?? 0} moves` : ''
);
...
check(
  ...
  b1.status === 'ok' && (b1.ranked?.length ?? 0) === 2,
  `top: ${b1.status === 'ok' ? b1.ranked?.[0]?.url : ''}`
);
...
check(
  ...
  b2.status === 'ok' && b2.strategy?.worthAttending === 'yes',
  b2.status === 'ok' ? `verdict=${b2.strategy?.worthAttending}` : ''
);
...
check(
  ...
  b4a.status === 'ok',
  b4a.status === 'ok' ? `formality=${b4a.brief?.formality}` : ''
);

const b4b = await expensePolicyCheckerTool.handler(
  {
    expenses: [
      { category: 'flight', amountUsd: 1200 },
      { category: 'hotel_per_night', amountUsd: 180 },
    ],
    policy: {
      flightCabin: 'economy',
      flightCapUsd: 2500,
...
check(
  ...
  b4b.status === 'ok' && b4b.withinPolicy === true,
  b4b.status === 'ok' ? `within=${b4b.withinPolicy}` : ''
);
...
check(
  ...
  b5.status === 'ok' && b5.verdict?.riskLevel === 'low',
  b5.status === 'ok' ? `risk=${b5.verdict?.riskLevel}` : ''
);
...
check(
  ...
  b6.status === 'ok' && b6.needsAdapter === true && b6.needsConverter === true
);
...
const b8 = await tripContextualRecommenderTool.handler(
  {
    city: 'Tokyo',
    situation:
      'I just landed after a 14h flight and I am exhausted, want something low-key tonight.',
  } as never,
  ctx
);
check(
  ...
  b8.status === 'ok' && b8.intent === 'low_key',
  b8.status === 'ok' ? `intent=${b8.intent}` : ''
);
...
check(
  ...
  b9a.status === 'ok',
  b9a.status === 'ok' ? `verdict=${b9a.verdict}` : ''
);
...
check(
  ...
  b9b.status === 'ok' && (b9b.alternatives?.length ?? 0) >= 1
);
    ...
    quotes: [
      {
        ...
        kind: 'transfer',
        priceUsd: 150,
        deliverables: ['airport pickup', 'wifi'],
        cancellationTerms: 'free up to 24h',
      },
      {
        supplier: 'Supplier B',
        kind: 'transfer',
        priceUsd: 200,
        deliverables: ['airport pickup'],
        depositPct: 75,
      },
...
check(
  ...
  b10a.status === 'ok' && b10a.cheapest?.supplier === 'Supplier C',
  b10a.status === 'ok' ? `cheapest=${b10a.cheapest?.supplier}` : ''
);

const b10b = await agencyMarginGuardTool.handler(
  {
    cogs: 1000,
    proposedPriceUsd: 1300,
    policy: { minMarkupBps: 500, maxMarkupBps: 3500, minMarginUsd: 50 },
  } as never,
  ctx
);
check(
  'B10',
  'agency_margin_guard',
  b10b.status === 'ok' && b10b.verdict === 'approved',
  b10b.status === 'ok' ? `verdict=${b10b.verdict} markup=${(b10b.markupBps ?? 0) / 100}%` : ''
);
